-- PRODUCTION
WITH LatestPlayerPassing AS (
    SELECT 
        ppas.*,  
        ppos.Position,
        ROW_NUMBER() OVER(PARTITION BY ppas.PlayerId ORDER BY ppas.GameDate DESC) AS RowNum
    FROM player_passing AS ppas
    JOIN player_positions AS ppos
        ON ppas.PlayerId = ppos.PlayerId
        AND ppas.Player = ppos.Player
        AND ppas.GameDate = ppos.GameDate
        AND ppas.TeamAbbr = ppos.TeamAbbr
    WHERE ppos.Position = "QB"
)

SELECT 
    -- latest-player-passing
    lppas.*,  -- Select all columns from player_rushing
...
FROM LatestPlayerPassing AS lppas
...
JOIN team_passing AS tpas
    ON lppas.GameId = tpas.GameId
    AND lppas.GameId2 = tpas.GameId2
    AND lppas.GameDate = tpas.GameDate
    AND lppas.TeamAbbr = tpas.TeamAbbr  

JOIN team_rushing AS trus
    ON lppas.GameId = trus.GameId
    AND lppas.GameId2 = trus.GameId2
    AND lppas.GameDate = trus.GameDate
    AND lppas.TeamAbbr = trus.TeamAbbr
    AND lppas.Season = trus.Season   

JOIN team_receiving AS trec
    ON lppas.GameId = trec.GameId
    AND lppas.GameId2 = trec.GameId2
    AND lppas.GameDate = trec.GameDate
    AND lppas.TeamAbbr = trec.TeamAbbr
    
JOIN team_plays AS tpla
    ON lppas.GameId = tpla.GameId
    AND lppas.GameId2 = tpla.GameId2
    AND lppas.GameDate = tpla.GameDate
    AND lppas.TeamAbbr = tpla.TeamAbbr    
    
JOIN league_passing AS lpas
    ON lppas.GameDate = lpas.GameDate  
    AND lppas.Season = lpas.Season   
 
JOIN league_rushing AS lrus
    ON lppas.GameDate = lrus.GameDate
    AND lppas.Season = lrus.Season     

JOIN league_receiving AS lrec
    ON lppas.GameDate = lrec.GameDate
    AND lppas.Season = lrec.Season   

JOIN league_plays AS lpla
    ON lppas.GameDate = lpla.GameDate      
    AND lppas.Season = lpla.Season  
     
WHERE lppas.RowNum = 1
